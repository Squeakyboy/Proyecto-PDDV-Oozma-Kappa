   ...
   private void Update()
{
    ...
    bool fire = controls.Gameplay.Shoot.ReadValue<float>() > 0.5f;

    if (controls.Gameplay.ChangeWeapon.triggered)
    {
        CambioArmaFunc();
    }
   ...
   if (fire && cont >= armaActual.tiempoEntreBalas)
{
    if (armaActual.balas > 0)
    {
        ...
        // Vibración al disparar
        if (Haptica.instance != null)
            Haptica.instance.Vibrar(0.4f, 0.7f, 0.1f);
    }
}


    bool recarga = Input.GetButton("Recargar");
    if (fire) { recarga = false; }
